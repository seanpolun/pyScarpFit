...
def dsp_scarp_identify(x, z):
    ...
    z_lower_reg = lower_model.predict(x_real_lower)
    z_upper_reg = upper_model.predict(x_real_upper)
    ...
    opt_h = (upper_model.intercept_ - lower_model.intercept_) / 2

    # IF mid-distance of scarp is not equal to lower intercept, adjust so that midpoint is at actual midpoint
    z_adj = opt_h - np.abs(lower_model.intercept_)
    if np.abs(z_adj) > 0.05:
        opt_midz = opt_midz + z_adj
        idx = (np.abs(z_new - opt_midz)).argmin()
        opt_midx = x_new[idx]

    return opt_midx, opt_midz, b1, b2, opt_h


def fit_1event(x, z, xmid, zmid, b1, b2, H_guess):
    H_unc = 0.15
    ...
    z1 = z1 - (x1 * b)
    opt_d = grid_search_d(scarp_1e, D_min, D_max, D_step, x1, z1, H_guess)
    D = uncertainties.ufloat(opt_d, (H_unc * opt_d))
    H = uncertainties.ufloat(H_guess, H_unc_abs)
    return H, D


def fit_ss_uplift(x, z, xmid, zmid, b1, b2, H_guess):
    H_unc = 0.15
    H_unc_abs = np.abs(H_guess * H_unc)
    x1 = x - xmid
    z1 = z - zmid
    ...
    opt_d = grid_search_d(scarp_ss, D_min, D_max, D_step, x1, z1, H_guess)
    D = uncertainties.ufloat(opt_d, (H_unc * opt_d))
    H = uncertainties.ufloat(H_guess, H_unc_abs)
    return H, D
...
class Scarp:
    def __init__(self, x, z, b_fit='dsp', name=''):
        ...
        if b_fit == 'dsp':
            self.midx, self.midz, b1, b2, self.Hinit = dsp_scarp_identify(self.x, self.z)
            self.b1 = uncertainties.ufloat(b1, self.default_unc)
            self.b2 = uncertainties.ufloat(b2, self.default_unc)
            # self.Hinit = 1.0
        else:
    ...
    def gen_1e(self):
        self.H1, self.D1 = fit_1event(self.x, self.z, self.midx, self.midz, self.b1.n, self.b2.n, self.Hinit)

    def gen_ss(self):
        self.Hs, self.Ds = fit_ss_uplift(self.x, self.z, self.midx, self.midz, self.b1.n, self.b2.n, self.Hinit)

    def plot_scarp(self, type):
        ...
        # H = self.Hinit
        if type == "se":
            H = self.H1.nominal_value
            D = self.D1.nominal_value
            scarp_model = scarp_1e(x1, H, D) + (x1 * self.b)
        elif type == "ss":
            H = self.Hs.nominal_value
            D = self.Ds.nominal_value
            scarp_model = scarp_ss(x1, H, D) + (x1 * self.b)
        else:
            raise ValueError("Undefined scarp type")

        ax.plot(x1, init_geom(x1, H, self.b), '--', c='darkgray')
        ...
        annot_text = name_text + model_text + H_text + D_text + b_text
